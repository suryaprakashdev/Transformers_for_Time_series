...
class MOMENT_TS(nn.Module):
    """
    MOMENT-style Transformer for Time-Series
    Improvements over MANTIS:
    - Positional Encoding
    - Better sequence representation
    - CLS token for global understanding
    """
    ...
    def __init__(self, c_in, c_out, seq_len):
        ...
        # Input embedding
        self.embedding = nn.Conv1d(c_in, self.d_model, kernel_size=5, padding=2)
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(self.d_model, c_out)
        )

    def forward(self, x):
        # x: (batch, channels, seq_len)

        x = self.embedding(x)                # (B, 128, L)
        ...
        x = torch.cat((cls_tokens, x), dim=1)          # (B, L+1, 128)
